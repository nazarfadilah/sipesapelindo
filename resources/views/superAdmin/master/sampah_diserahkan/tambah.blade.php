...
@section('title', 'Tambah Sampah Diserahkan')
...
<div class="content-area-form">
    ...
    <div class="card">
        <div class="card-body bg-primary text-white">
            <h5 class="mb-0">Tambah Sampah Diserahkan</h5>
        </div>
        
        <div class="card-body">
            <form action="{{ route('superadmin.master.sampah-diserahkan.store') }}" method="POST" id="sampahDiserahkanForm">
                @csrf
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('id_jenis') is-invalid @enderror" id="id_jenis" name="id_jenis" required>
                            ...
                            @foreach($jenises as $jen)
                                <option value="{{ $jen->id }}" {{ old('id_jenis') == $jen->id ? 'selected' : '' }}>{{ $jen->nama_jenis }}</option>
                            @endforeach
                        </select>
                        @error('id_jenis')
                        <div class="invalid-feedback">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('id_diserahkan') is-invalid @enderror" id="id_diserahkan" name="id_diserahkan" required>
                            ...
                            @foreach($tujuanSampahs as $tuj)
                                <option value="{{ $tuj->id }}" {{ old('id_diserahkan') == $tuj->id ? 'selected' : '' }}>{{ $tuj->nama_tujuan }}</option>
                            @endforeach
                        </select>
                        @error('id_diserahkan')
                        <div class="invalid-feedback">
...
@push('scripts')
<script>
    setupFormConfirmation('sampahDiserahkanForm', 'Apakah Anda yakin ingin menyimpan data sampah diserahkan ini?');
</script>
@endpush
